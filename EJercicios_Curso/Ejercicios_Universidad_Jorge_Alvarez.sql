...
--4. Devuelve el listado de profesores que no han dado de alta su número de teléfono en la base de 
--datos y además su nif termina en K.
...
--7.4 Consultas multitabla (Composición interna); 
...
WHERE  G.nombre IN ('Grado en Ingeniería Informática (Plan 2015)') AND P.sexo IN ('M')
...
JOIN asignatura ASS ON ASS.id = A.id_asignatura
JOIN grado G ON G.id = ASS.id_grado
WHERE  G.nombre IN ('Grado en Ingeniería Informática (Plan 2015)') 
GROUP BY  ASS.nombre
...
--y nombre del departamento. El resultado estará ordenado alfabéticamente de menor a mayor por 
...
JOIN grado G ON G.id = ASS.id_grado
WHERE ASS.nombre IN HERE  G.nombre IN ('Grado en Ingeniería Informática (Plan 2015)') 

--6. Devuelve un listado con todos los alumnos que se han matriculado en alguna asignatura durante 
--el curso escolar 2018/2019.--7.5 Consultas multitabla (Composición externa); 
...
--3. Calcula cuántos profesores hay en cada departamento. El resultado sólo debe mostrar dos 
...
--4. Devuelve un listado con todos los departamentos y el número de profesores que hay en cada uno 
...
--asignaturas asociadas. Estos grados también tienen que aparecer en el listado. El resultado deberá 
--estar ordenado de mayor a menor por el número de asignaturas.
--6. Devuelve un listado con el nombre de todos los grados existentes en la base de datos y el número 
...
--tipo de asignatura y la suma de los créditos de todas las asignaturas que hay de ese tipo. Ordene 
--el resultado de mayor a menor por el número total de créditos.
--8. Devuelve un listado que muestre cuántos alumnos se han matriculado de alguna asignatura en 
--cada uno de los cursos escolares. El resultado deberá mostrar dos columnas, una columna con el 
--año de inicio del curso escolar y otra con el número de alumnos matriculados.
--9. Devuelve un listado con el número de asignaturas que imparte cada profesor. El listado debe tener 
--en cuenta aquellos profesores que no imparten ninguna asignatura. El resultado mostrará cinco 
--columnas: id, nombre, primer apellido, segundo apellido y número de asignaturas. El resultado 
