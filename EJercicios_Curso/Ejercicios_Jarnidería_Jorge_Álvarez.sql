...
CREATE TABLE producto (
 ...
 precio_proveedor DECIMAL DEFAULT NULL,
...
-- 6. Devuelve un listado con el nombre de los todos los clientes espan~oles.

SELECT C.nombre_cliente, C.pais
FROM cliente C
WHERE C.pais IN ('SPAIN')
...
-- 8. Devuelve un listado con el co´digo de cliente de aquellos clientes que realizaron algún pago en 
-- 2008. Tenga en cuenta que deberá eliminar aquellos códigos de cliente que aparezcan repetidos. 

SELECT DISTINCT P.codigo_cliente
FROM pago P
WHERE YEAR(P.fecha_pago) = 2008

-- 9. Devuelve un listado con el código de pedido, co´digo de cliente, fecha esperada y fecha de entrega 
...
-- 10. Devuelve un listado con el código de pedido, co´digo de cliente, fecha esperada y fecha de entrega 
-- de los pedidos cuya fecha de entrega ha sido al menos dos di´as antes de la fecha esperada.
...
SELECT P.codigo_pedido, P.codigo_cliente, P.fecha_esperada, P.fecha_entrega
FROM pedido P
WHERE DAY(P.fecha_esperada) - DAY(P.fecha_entrega) >= 2 and month(P.fecha_esperada) =  month(P.fecha_entrega) AND
	  YEAR(P.fecha_esperada) =  YEAR(P.fecha_entrega)
	--no se puede
...
SELECT P.estado
FROM PEDIDO P
WHERE P.estado IN ('Rechazado')

-- 12. Devuelve un listado de todos los pedidos que han sido entregados en el mes de enero de cualquier 
-- año.

SELECT P.codigo_pedido,P.estado,P.fecha_entrega
FROM PEDIDO P
WHERE P.estado IN ('Entregado') and MONTH(P.fecha_entrega) IN (1)
ORDER BY 1
...
SELECT *
FROM pago P
WHERE P.forma_pago IN ('PayPal') AND YEAR(P.fecha_pago) IN (2008)
...
FROM producto P
WHERE P.gama IN ('Ornamentales') AND P.cantidad_en_stock >100
...
SELECT *
FROM cliente C
WHERE C.ciudad IN ('MADRID')  AND C.codigo_empleado_rep_ventas IN (11,30)

--6.4 Consultas multitabla (Composición interna)

--Resuelva todas las consultas utilizando la sintaxis de SQL1 y SQL2. Las consultas con sintaxis de SQL2 se 
--deben resolver con INNER JOIN y JOIN.
...
SELECT C.nombre_cliente, E.nombre AS NOMBRE_EMPLEADO ,E.apellido1, P.codigo_cliente
FROM CLIENTE C
JOIN empleado E ON E.codigo_empleado = C.codigo_empleado_rep_ventas
JOIN pago P ON P.codigo_cliente = E.codigo_empleado
--REVISAR

--3. Muestra el nombre de los clientes que no hayan realizado pagos junto con el nombre de sus 
--representantes de ventas.

SELECT C.nombre_cliente,E.nombre AS NOMBRE_REPRESENTANTE_VENTAS
FROM CLIENTE C
JOIN empleado E ON E.codigo_empleado = C.codigo_empleado_rep_ventas
FULL OUTER JOIN pago P ON P.codigo_cliente = c.codigo_cliente
...
SELECT DISTINCT C.nombre_cliente, E.nombre AS Nombre_representante, O.ciudad
FROM cliente C
FULL OUTER JOIN pago P ON C.codigo_cliente = p.codigo_cliente
JOIN empleado E ON E.codigo_empleado = C.codigo_empleado_rep_ventas
JOIN oficina O ON O.codigo_oficina = E.codigo_oficina
ORDER BY 1, 2,3
...
SELECT DISTINCT C.nombre_cliente, E.nombre AS NOMBRE_REPRESENTANTE, O.ciudad AS CIUDAD_OFICINA_REPRESENTANTE
FROM cliente C
JOIN empleado E ON E.codigo_empleado = C.codigo_empleado_rep_ventas
JOIN oficina O ON O.codigo_oficina = E.codigo_oficina
WHERE C.ciudad IN ('FUENLABRADA')
...
SELECT E2.nombre AS NOMBRE_EMPLEADO, E.nombre AS NOMBRE_JEFE 
FROM empleado E
...
SELECT E2.nombre AS NOMBRE_EMPLEADO, E.nombre AS NOMBRE_JEFE, E.codigo_jefe, E3.nombre AS NOMBRE_JEFE_DE_JEFE
FROM empleado E
JOIN empleado E2 ON E.codigo_empleado = E2.codigo_jefe
JOIN empleado E3 ON E3.codigo_empleado = E.codigo_jefe
...
--6.5 Consultas multitabla (Composición externa)
...
WHERE pe.fecha_pedido IS NULL AND P.fecha_pago IS NULL


--4. Devuelve un listado que muestre solamente los empleados que no tienen una oficina asociada.


SELECT E.nombre , E.codigo_oficina 
FROM empleado E
...
--15. La facturación que ha tenido la empresa en toda la historia, indicando la base imponible, el IVA y 
--el total facturado. La base imponible se calcula sumando el coste del producto por el número de 
...
--6.7.1 Con operadores básicos de comparación; 
...
--6.7.2 Subconsultas con ALL y ANY; 
...
--6.7.3 Subconsultas con IN y NOT IN; 
...
--6.7.4 Subconsultas con EXISTS y NOT EXISTS; 
...
--6.8 Subconsultas correlacionadas; 

--6.8.1 Consultas variadas; 
